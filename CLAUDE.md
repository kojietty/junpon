...
- **何を作るか**: 中央に積まれたノーツを **S / D / J / K の 4 キー**で下から達磨落とし式に消していくアーケード Web ゲーム（コードネーム: **JUNGLE TAP**）
- **メカニクス**: スタック最下段のノーツのタイプ (4 種) に対応するキーを押すと消える。不正解は **スコア -1 ペナルティ** (床 0)。60 秒の制限時間でスコアを稼ぐ
- **配信先**: Web ブラウザ（PC / モバイル）
- **スコープ**: 元は LINE Friends の Android ゲーム「JUNGLE PANG」をベースに左右 2 キーで開発開始したが、2026-05-15 に 4 キー化へ転向。キャラクターは独自意匠（著作権配慮）
